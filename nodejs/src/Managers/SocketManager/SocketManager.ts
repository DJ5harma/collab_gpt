...
export class SocketManager {
	...
	static init(io: Server) {
		...
		this.io.on("connection", (socket) => {
			...
			socket.on("accepted-room-join-request", ({ room_id, user, access }) => {
				RoomsManager.get_room(room_id)
					?.members.filter(({ access }) => access === RoomAccess.ADMIN)
					.forEach((member) => {
						if (socket === this.userid_to_socket.get(member.user.id)) {
							this.userid_to_socket.get(user.id)?.join(room_id);
							RoomsManager.add_user_to_room(user, room_id, access);
							return;
						}
					});
			});
			...
			socket.on(
				...
				({
					...
					RoomsManager.get_room(room_id)
						?.members.filter(
							({ access }) =>
								access === RoomAccess.ADMIN || access === RoomAccess.WRITE
						)
						.forEach((member) => {
